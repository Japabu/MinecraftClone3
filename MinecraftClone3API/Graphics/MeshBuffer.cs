...
namespace MinecraftClone3API.Graphics
{
    ...
    /// ChunkMesher"/> on the mesh thread). Holds no GPU state, so building it is safe off the render thread.
    ...
    public class MeshBuffer
    {
        ...
        public List<Vector3D<float>> Positions;   // attrib 0: 3 x float
        public List<Vector2D<float>> Uvs;         // attrib 1: 2 x float
        ...
        /// path). Overridden by the GPU <c>SortedVertexArrayObject</c>; lets the Core mesher branch on sorting
        /// without referencing the client-only VAO type.</summary>
        public virtual bool Sorted => false;

        public virtual void Add(Vector3D<float> position, Vector4D<float> texCoord, Vector4D<float> normal, Vector3D<float> color, Vector4D<float> light)
        {
            if (Positions == null)
            {
            ...
            Uvs.Add(new Vector2D<float>(texCoord.X, texCoord.Y));
        ...
        private static uint PackVertex(Vector4D<float> texCoord, Vector4D<float> normal)
        {
        ...
        public virtual void AddFace(uint[] indices, Vector3D<float> faceMiddle) => Indices.AddRange(indices);
        ...
        public virtual void AddFace(int baseVertex, bool flipped, Vector3D<float> faceMiddle)
        {
